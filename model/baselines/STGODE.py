...
class STGODE(nn.Module):
    ...
    def __init__(self,  edge_index: 'edge_index', edge_weight: 'edge_weights', num_features, num_timesteps_input,
        ...
        A_sp_hat = torch.sparse_coo_tensor(edge_index,
                                           edge_weight, torch.Size([num_nodes, num_nodes])).to_dense()
        A_se_hat = torch.sparse_coo_tensor(edge_index, torch.where(edge_weight > 0, 1.0, 0.0),
                                           torch.Size([num_nodes, num_nodes])).to_dense()
        self.sp_blocks = nn.ModuleList(
            [nn.Sequential(
                STGCNBlock(in_channels=num_features, out_channels=out_channels,
                           num_nodes=num_nodes, A_hat=A_sp_hat),
                STGCNBlock(in_channels=out_channels[-1], out_channels=out_channels,
        ...
        self.pred = nn.Sequential(
            nn.Linear(num_timesteps_input * out_channels[-1], num_timesteps_output * 32),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        return self.pred(x).unsqueeze(-1)