...
from .layers.abstract_traffic_state_model import AbstractTrafficStateModel
...
class PDFormer(nn.Module):
    def __init__(self, edge_index:'edge_index'):
        super().__init__()
        self.name = 'PDFormer'
        ...
        self.world_size = 1
        self.huber_delta = 1
        self.quan_delta = 0.25
        self.far_mask_delta = 5
        self.dtw_delta = 5
        ...
        self.sem_mask = None
        self.geo_mask = None
        # self.sem_mask = np.load('./dataset/PDFormer/dtw_PeMS04.npy')
        pattern_keys = np.load('./dataset/PDFormer/pattern_keys_kshape_PeMS04_14_3_16_5.npy')
        # pattern_keys = np.random.rand(16, 3, 1)
        ...
        self.enc_embed_layer = DataEmbedding(
            self.feature_dim - self.ext_dim, self.embed_dim, lape_dim, drop=drop,
