...
class SCINet(nn.Module):
    def __init__(self, output_len, input_len, edge_index:'edge_index', hid_size=1, num_stacks=1,
                 num_levels=3, num_decoder_layer=1, concat_len=0, groups=1, kernel=5, dropout=0.5,
                 single_step_output_One=0, positionalE=True, modified=True, RIN=True):
        super(SCINet, self).__init__()
        self.name = 'SCINet'
        input_dim = edge_index.max().item() + 1
