...
class DSTAGNN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
                 cheb_polynomials, adj_pa, adj_TMD, num_for_predict, len_input, num_of_vertices, d_model, d_k, d_v,
                 n_heads):
        '''
        :param nb_block:
        :param in_channels:
...
class DSTAGNN(nn.Module):
    def __init__(self, edge_index: 'edge_index', edge_weight: 'edge_weights', nb_block, in_channels,
                 nb_chev_filter, nb_time_filter, time_strides, num_for_predict, len_input, d_model, d_k, n_heads):
        nn.Module.__init__(self)
        self.name = 'DSTAGNN'
        num_of_vertices = edge_index.max().item() + 1
        num_of_d = in_channels
        adj_mx = torch.sparse_coo_tensor(edge_index,
                                         edge_weight, torch.Size([num_of_vertices, num_of_vertices])).to_dense().numpy()
        adj_TMD = adj_mx
        adj_pa = adj_mx
        ...
        self.real_model = DSTAGNN_submodule(DEVICE, num_of_d, nb_block, in_channels,
                                  3, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials,
                                  adj_pa, adj_TMD, num_for_predict, len_input, num_of_vertices, d_model, d_k, d_k,
                                  n_heads)
