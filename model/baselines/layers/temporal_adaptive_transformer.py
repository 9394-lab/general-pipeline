...
class AdaptiveTransformer(nn.Module):
    ...
    def __init__(self, epsilon, flag, hidden_size, max_step, num_heads, seq_length=100, input_dropout=0.0,
                 layer_dropout=0.0,
    ...
    def forward(self, inputs, STE, mask):
        ...
        if self.dhm:
            x, (remainders, n_updates) = self.dhm_fn(x, inputs, STE, self.att, self.position_embedding,
                                                     self.recurrent_embedding, self.max_step, mask)
...
class DHM_basic(nn.Module):
    ...
    def forward(self, state, inputs, STE, fn, pos_enc, recur_enc, max_step, mask, encoder_output=None):
        ...
        while ((halting_probability < self.threshold) & (n_updates < max_step)).byte().any():
            ...
            previous_state = ((state.cuda() * update_weights.unsqueeze(-1).cuda()) + (
                        previous_state.cuda() * (1 - update_weights.unsqueeze(-1)).cuda()))
