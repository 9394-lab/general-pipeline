...
class SimST(nn.Module):
    def __init__(self, device, edge_index:'edge_index', node_dim, in_dim, out_dim, init_dim, end_dim, layer, dropout):
        ...
        if node_dim != 0:
            ...
            print('Random init trainable node embed')
    ...
    def forward(self, input, idx=None):  # input (bs, hid, node, time)
        x = self.start_conv(input.permute(0, 3, 1, 2))
