...
class IDGCN(nn.Module):
    def __init__(self, device, channels, splitting=True, num_nodes=1844, dropout=0.25, pre_adj=None, pre_adj_len=1):
...
class STIDGCN(nn.Module):
    def __init__(self, input_channel, device, edge_index: 'edge_index', edge_weight: 'edge_weights', channels, dropout=0.25):
        super().__init__()
        self.name = 'STIDGCN'
        self.dropout = dropout
        ...
        pre_adj = torch.sparse_coo_tensor(edge_index, edge_weight, [num_nodes, num_nodes]).to_dense()
        pre_adj = [torch.tensor(asym_adj(pre_adj)).to(device),
                   torch.tensor(asym_adj(np.transpose(pre_adj))).to(device)]
